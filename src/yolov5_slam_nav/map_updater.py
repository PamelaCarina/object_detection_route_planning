...
class MapUpdater(Node):
    def __init__(self):
        ...
        # no lo estoy utilizando porque la distancia la obtengo desde yolov5
    ...
    def yolo_callback(self, msg):
        ...
        for detection in detections:
            ...
            if obj_name in self.objects_db:
                self.get_logger().info(f"Objeto '{obj_name}' ya está registrado, se omite.")
                continue
    ...
    def object_to_map(self, x_obj, y_obj):
        #Convierte coordenadas del robot a coordenadas globales usando TF
        try:
            # vector posición del objeto que detecto, definido en el frame de la base
            #vector_local = Vector3Stamped()
            #vector_local.header.frame_id = "base_link"
            #vector_local.header.frame_id = "base_footprint"
            ...
            point_local.point.x = x_obj
            point_local.point.y = y_obj
            point_local.point.z = 0.0

            # obtener posicion del robot respecto al origen
            # obtienes la pose del robot, la cual es una matrix de transformación que define donde está la base del robot (‘base_footprint’) 
            ...
            #transform = self.tf_buffer.lookup_transform(
            #    'map', 'base_footprint', tf2_ros.Time(),
            #    timeout=rclpy.duration.Duration(seconds=0.5)
            #)

            # Transformamos el vector al frame del mapa (respeta la rotación)
            # transformas ‘vector_global’ al frame del mapa
            #vector_global = do_transform_vector3(vector_local, transform)

            #self.get_logger().info(f"Transformación: {transform.transform}")


            # Obtener las coordenadas base del robot
            # esto debería ser cero cuando recién parte
            ...
            #solución que da matías:
            #x_obj_map = vector_global.vector.x
            #y_obj_map = vector_global.vector.y

            # Aplica el transform correctamente
            point_global = do_transform_point(point_local, transform)

            x_obj_map = point_global.point.x
            y_obj_map = point_global.point.y
